...
function copy_deployment_folder() {
  ...
  cp -rv "$DEPLOYMENT_DIR"/images "$OPENSRC_DIST_DIR"/deployment/images
  cp -rv "$DEPLOYMENT_DIR"/samples "$OPENSRC_DIST_DIR"/deployment/samples
...
function copy_source_folder() {
  ...
  rm -v ./webapp/solution-manifest.js
  popd
}
